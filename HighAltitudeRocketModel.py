...
Fg_approx_curve = np.zeros(num_of_intervals)
for i in range(num_of_intervals):
    Fg_approx_curve[i] = 9.8 * np.interp(time_approx_curve[i], time, mass)
...
for i in range(num_of_intervals):
    ...
    instantaneous_Fg = Fg_approx_curve[i]
    instantaneous_drag = 0.5 * air_density_curve[i] * (instantaneous_velocity ** 2.0) * Cd * ref_area
    instantaneous_net_force = instantaneous_thrust - instantaneous_drag - instantaneous_Fg
